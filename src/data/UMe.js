...
export class UMe {
    ...
    constructor(data) {
        ...
        const putAnswer = (answerObject, key, value) => {

            if (!value) return

            answerObject[key] = value.map(a => a.toString())

        }

        this.name = data[_Config.nameField]
        _Config.startEnquiryFields.forEach(e => putAnswer(this.startEnquiryAnswer, e, data[e]))
        _Config.commonEnquiryFields.forEach(e => putAnswer(this.enquiryAnswer, e, data[e]))
